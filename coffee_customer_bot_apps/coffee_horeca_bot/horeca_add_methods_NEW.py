...
class HorecaBotMethods:
    ...
    async def get_data_by_user_id(self, user_id) -> [list]:
        ...
        return json.loads(requests.get(url).content.decode('utf-8'))['response']
    ...
    async def send_short_cards(self, **kwargs):
        ...
        for order in orders:
            if order['status'] == 'canceled_by_cafe':
                pass
            buttons = variables.first_short_inline_buttons if list(variables.BARISTA_STATUS_CHOICES.keys()).index(order['status']) == 0 else variables.between_short_inline_buttons
